...
const VideoMeet = () => {
  ...
  // ------------------ Helpers for local media management ------------------
  const silence = () => {
    let ctx = new AudioContext();
    let oscillator = ctx.createOscillator();
    let dst = oscillator.connect(ctx.createMediaStreamDestination());
  ...
  // Create a dummy stream (black video + silent audio)
  const getDummyMediaStream = () => {
    return new MediaStream([black(), silence()]);
  };

  // Start or update the local media stream according to current video/audio state
  const startLocalMedia = async () => {
    ...
    try {
      let stream;
      // If either video or audio requested and available, getUserMedia
      if ((video && videoAvailable) || (audio && audioAvailable)) {
        stream = await navigator.mediaDevices.getUserMedia({
          video: video && videoAvailable ? { width: 1280, height: 720 } : false,
          audio: audio && audioAvailable ? true : false
        });
        // If one type missing, add dummy track so peer connections always have both kinds
        if (video && !audio) {
        ...
        // If both turned off, use dummy stream
      ...
      updateConnectionTracks(stream);

      // Attach onended to tracks to revert to dummy when user stops camera/sharing
      if (stream && stream.getTracks) {
        stream.getTracks().forEach(track => {
          track.onended = () => {
            // when user toggles or screen share ends etc., fallback to dummy
            try {
              if (localVideoRef.current && localVideoRef.current.srcObject) {
                localVideoRef.current.srcObject.getTracks().forEach(t => t.stop());
              }
            } catch (e) { /* ignore */ }
            window.localStream = getDummyMediaStream();
            if (localVideoRef.current) localVideoRef.current.srcObject = window.localStream;
            updateConnectionTracks(window.localStream);
          };
        });
      }
      ...
      console.warn('getUserMedia failed, using dummy stream', err);
      const dummy = getDummyMediaStream();
      window.localStream = dummy;
      if (localVideoRef.current) localVideoRef.current.srcObject = dummy;
      updateConnectionTracks(dummy);
      return dummy;
    }
  };

  // Replace/attach tracks on all RTCPeerConnections
  const updateConnectionTracks = (stream = window.localStream) => {
    if (!stream) return;
    Object.keys(connections).forEach(id => {
      ...
      try {
        ...
        console.warn('updateConnectionTracks error for', id, e);
      }
    });
  };
  ...
  const getUserMediaSuccess = (stream) => {
    // this function is still used in some flows — keep for compatibility
    try {
      if (window.localStream && window.localStream.getTracks) {
        window.localStream.getTracks().forEach(track => track.stop());
      }
  ...
  const getUserMedia = () => {
    // kept for compatibility; will call startLocalMedia to actually fetch stream
    if ((video && videoAvailable) || (audio && audioAvailable)) {
      startLocalMedia().then(getUserMediaSuccess).catch(e => console.log(e));
    } else {
      try {
        if (localVideoRef.current && localVideoRef.current.srcObject) {
          let tracks = localVideoRef.current.srcObject.getTracks();
  ...
  const getMedia = () => {
    // Connect to socket server (local media already started in connect)
    connectToSocketServer();
  };

  const routeTo = useNavigate();

  // IMPORTANT: connect() now starts local media first so permission prompt happens
  const connect = async () => {
    if (!username.trim()) {
      setUsernameError('Username is required');
      return;
    ...
    // start media (based on video/audio toggles)
    await startLocalMedia();
    setAskForUsername(false);
    getMedia();
  };

  // When toggling video / audio, update the local media and tracks
  useEffect(() => {
    // don't call on initial render (askForUsername true). Only when user toggles after entering
    if (!askForUsername) {
      // startLocalMedia will handle dummy stream fallback
      startLocalMedia().catch(e => console.log('startLocalMedia error', e));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [video, audio]);
    ...
    <div className="videomeet-wrapper">
      {askForUsername === true ? (
        ...
        <div className="videomeet-container">
          ...
          {/* Participants Sidebar, Chat overlays unchanged */}
          {showParticipants && (
            <div className="videomeet-participants-overlay">
              <div className="videomeet-participants-glass">
                <div className="participants-header">
